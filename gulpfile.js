...
// console.log('this.currentRunTaskName: ' + this.currentRunTaskName); Live
// reload socket for sandbox area var io = require('socket.io')(8889);
// gulp.watch(['samples/**/*'], () => {   io.emit('reload'); });

gulp.task('new', [
  'js', 'css'
], function () {
  gulp
    .src(['webpack.config.js', 'webpack.config.dev.js', 'package.json', 'karma.config.js'])
...
gulp.task('clean-dst', function (callback) {
  del([
    'dst/*.js', 'dst/*.js.map', '!dst/vendor.js'
  ], {force: true})
    .then(function () {
      callback();
    });
...
gulp.task('webpack-dev-server', function (callback) {
  ...
  myConfig
    .entry['abl-sdk']
    .unshift('webpack-dev-server/client?http://0.0.0.0:' + port + '/', 'webpack/hot/dev-server');
